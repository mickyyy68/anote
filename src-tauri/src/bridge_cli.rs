...
fn delete_note(conn: &Connection, payload: DeleteNotePayload) -> Result<Value, BridgeResponse> {
    ...
    let rows = if let Some(expected_updated_at) = payload.updated_at {
        // Keep stale-delete protection atomic with the write itself to avoid races with concurrent editors.
        conn.execute(
            "DELETE FROM notes WHERE id = ?1 AND updated_at <= ?2",
            params![&payload.id, expected_updated_at],
        )
        .map_err(|e| err("INTERNAL", e.to_string()))?
    } else {
        conn.execute("DELETE FROM notes WHERE id = ?1", params![&payload.id])
            .map_err(|e| err("INTERNAL", e.to_string()))?
    };
    ...
    if rows == 0 {
        ...
        return Err(err("CONFLICT", "stale note delete rejected"));
    }

    Ok(json!({ "id": payload.id }))
}
...
pub fn maybe_run_from_args() -> bool {
    let is_bridge_mode = env::args().skip(1).any(|arg| arg == "--bridge-cli");
    if !is_bridge_mode {
        return false;
    }
    run_stdio_from_stdin();
    true
}
...
mod tests {
    ...
    fn unique_test_db_path() -> PathBuf {
        std::env::temp_dir().join(format!(
            "anote_bridge_cli_test_{}_{}.db",
            std::process::id(),
            now_nanos()
        ))
    }

    fn cleanup_test_db_files(path: &Path) {
    ...
    fn open_test_db(path: &Path) -> Connection {
        let conn = db::open_initialized_db(path).expect("test db should initialize");
        conn.busy_timeout(Duration::from_secs(2))
            .expect("busy timeout should apply");
        conn
    }

    fn seed_note(conn: &Connection, note_id: &str, updated_at: i64) {
        conn.execute(
            "INSERT INTO folders (id, name, created_at, parent_id, updated_at) VALUES (?1, 'Inbox', 1, NULL, 1)",
            params!["folder1"],
        )
        .expect("folder insert should succeed");

        conn.execute(
            "INSERT INTO notes (id, folder_id, title, body, created_at, updated_at, pinned, sort_order) VALUES (?1, 'folder1', 't', 'b', 1, ?2, 0, 0)",
            params![note_id, updated_at],
        )
        .expect("note insert should succeed");
    }

    fn error_code(resp: &BridgeResponse) -> Option<&str> {
        resp.error.as_ref().map(|e| e.code.as_str())
    }

    #[test]
    fn delete_note_rejects_stale_when_newer_commit_wins_race() {
        let db_path = unique_test_db_path();
        cleanup_test_db_files(&db_path);

        {
            let conn = open_test_db(&db_path);
            seed_note(&conn, "note1", 100);
        }

        let writer = open_test_db(&db_path);
        writer
            .execute_batch("BEGIN IMMEDIATE")
            .expect("writer txn should start");
        writer
            .execute("UPDATE notes SET updated_at = 200 WHERE id = 'note1'", [])
            .expect("writer update should apply");

        let db_path_for_delete = db_path.clone();
        let (started_tx, started_rx) = mpsc::channel();
        let delete_handle = thread::spawn(move || {
            let conn = open_test_db(&db_path_for_delete);
        ...
        started_rx
            ...
            .expect("delete worker should start");
        thread::sleep(Duration::from_millis(100));

        writer
            .execute_batch("COMMIT")
            .expect("writer txn should commit");

        let delete_result = delete_handle
            .join()
            .expect("delete worker should not panic")
            .expect_err("stale delete should be rejected");
        assert_eq!(error_code(&delete_result), Some("CONFLICT"));

        let verify = open_test_db(&db_path);
        let remaining: (i64, i64) = verify
            .query_row(
                "SELECT COUNT(1), COALESCE(MAX(updated_at), 0) FROM notes WHERE id = 'note1'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .expect("verification query should succeed");
        assert_eq!(remaining.0, 1);
        assert_eq!(remaining.1, 200);

        cleanup_test_db_files(&db_path);
    }

    #[test]
    fn delete_note_without_timestamp_returns_not_found_for_missing_note() {
        let db_path = unique_test_db_path();
        cleanup_test_db_files(&db_path);
        ...
        let result = delete_note(
            &conn,
            DeleteNotePayload {
                id: "missingnote1".to_string(),
                updated_at: None,
            },
        );

        let resp = result.expect_err("missing note delete should fail");
